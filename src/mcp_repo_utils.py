...
Handles path resolution, extracting roots from MCP client context, and
coordinating fallback logic.
"""
...
def get_repo_root_from_path(start_path: Path) -> Path:
    ...
    Bees does not require a git repository and does not traverse the
    directory tree looking for one.
