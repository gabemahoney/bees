...
async def _create_ticket(
    ...
    with repo_root_context(resolved_root):
        ...
        config_path = resolved_root / '.bees/config.json'
        ...
        if not config or normalized_hive not in config.hives:
            ...
            # If MCP server cwd != resolved_root, this may indicate a context issue
            if ctx and cwd != resolved_root:
                # Check if there's a config in cwd that has the hive
                cwd_config_path = cwd / '.bees/config.json'
                if cwd_config_path.exists():
                    cwd_config = load_bees_config()
                    if cwd_config:
                        error_msg += (
                            f"\n"
                            f"NOTE: MCP server is running from {cwd} but detected client repo at {resolved_root}.\n"
        ...
        if not hive_resolved_path.exists():
            error_msg = f"Hive path does not exist: '{hive_resolved_path}'. Please create the directory before creating tickets."
            logger.error(error_msg)
            raise ValueError(error_msg)
        ...
        test_file = hive_resolved_path / f".write_test_{uuid.uuid4().hex[:8]}"
        try:
            test_file.touch()
            test_file.unlink()
            ...
            error_msg = f"Failed to write to hive directory '{hive_resolved_path}': {e}"
            logger.error(error_msg)
            raise ValueError(error_msg)
    ...
    with repo_root_context(resolved_root):
        ...
        if ticket.children:
            for child_id in ticket.children:
                try:
                    await _delete_ticket(child_id, ctx, repo_root)
...
async def _show_ticket(ticket_id: str, ctx: Context | None = None, repo_root: str | None = None) -> Dict[str, Any]:
    """
