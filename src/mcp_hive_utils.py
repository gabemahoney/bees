...
def validate_hive_path(path: str) -> Path:
    ...
    if not hive_path.is_absolute():
        raise ValueError(f"Hive path must be absolute, got relative path: {path}")
    ...
    if not hive_path.parent.exists():
        try:
            ...
            raise ValueError(f"Failed to create parent directory {hive_path.parent}: {e}") from e
    ...
    try:
        ...
        raise ValueError(
            f"Hive path must be within repository root. Path: {resolved_hive}, Repo root: {resolved_repo}"
        ) from None
...
def scan_for_hive(name: str, config: BeesConfig | None = None) -> Path | None:
    ...
    if config:
        ...
        for scope_data in global_config.get("scopes", {}).values():
            registered_hives.update(scope_data.get("hives", {}).keys())
