...
- **No caches**: Every operation reads from the filesystem directly. This ensures consistency and eliminates cache invalidation complexity. Performance remains acceptable by constraining scale.
...
These constraints trade performance for simplicity and reliability. By eliminating databases, daemons, and caches, we avoid entire classes of bugs (cache invalidation, daemon crashes, database corruption, connection pooling) while maintaining a system that's easy to understand, debug, and deploy.
...
- Architecture overview and module boundaries: `docs/plans/master_plan.md`
