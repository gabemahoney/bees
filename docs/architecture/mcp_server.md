...
- **create_ticket**: Create new ticket (bee or tier types) with automatic relationship synchronization. Requires `hive_name` parameter. The `body` parameter is constrained by the input schema to `maxLength: 10000` characters; for bodies longer than that, create the ticket with a short stub body and use `append_ticket_body` to write the rest in chunks. Accepts `body_file` (path to a UTF-8 file) as an alternative to `body`; `body` and `body_file` are mutually exclusive. The same 10000-character cap applies to `body_file`. Stdin (`"-"`) is not supported for `body_file` in the MCP context — pass content inline instead. See "Chunked body workflow" below.
