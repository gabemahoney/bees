...
The linter validates ticket schema compliance and relationship consistency, providing structured error reporting to identify database corruption issues. Index generation creates per-hive markdown indexes with validation. These systems work together to maintain data integrity.

For detailed architecture including linter algorithm, data structure design choices, index generation strategy, and corruption detection patterns, see [docs/architecture/validation.md](../architecture/validation.md).

**Key Integration Points**:
- Linter uses `src/reader.py` and `src/models.py` for ticket loading
- Index generation via `src/index_generator.py` creates per-hive indexes at `{hive_path}/index.md`
- Corruption state persisted to `.bees/corruption_report.json`
- MCP server `sanitize_hive` tool runs linter with auto-fix capability
