...
**Name Normalization**: Display names are normalized using `normalize_hive_name()` (see `docs/architecture/configuration.md` for normalization rules).

**Uniqueness Check**: Verifies normalized name doesn't exist in registry using `validate_unique_hive_name()`.

**Directory Structure Creation**: Creates `/eggs`, `/evicted`, and `.hive/identity.json` marker file with hive metadata.

**Config Registration**: Registers hive in `.bees/config.json` using config module functions (see `docs/architecture/configuration.md`).
