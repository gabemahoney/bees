...
- **Ticket ID validation** (`is_valid_ticket_id()`): 5 essential cases covering valid format, invalid prefix/suffix, separator edge cases, and boundary conditions
- **Port validation** (`Config.http_port`): 2 essential cases covering (1) valid port range 1024-65535 with type coercion, and (2) invalid ports including out-of-range and non-numeric values

This focused approach provides comprehensive coverage for simple validation functions without testing every possible invalid permutation.
