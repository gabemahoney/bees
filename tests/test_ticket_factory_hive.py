...
class TestCreateEpicWithHive:
    ...
    def test_create_epic_requires_context(self, temp_hive_config):
    ...
    def test_create_epic_with_hive(self, temp_hive_config):
        ...
        with repo_root_context(temp_hive_config):
            epic_id = create_epic(
                title="Backend Epic",
...
class TestCreateTaskWithHive:
    ...
    @pytest.mark.no_repo_context
    def test_create_task_requires_context(self, temp_hive_config):
        """Should raise RuntimeError when repo_root context is not set."""
        with pytest.raises(RuntimeError, match="repo_root not set in context"):
            create_task(
                title="Test Task",
...
class TestCreateSubtaskWithHive:
    ...
    @pytest.mark.no_repo_context
    def test_create_subtask_requires_context(self, temp_hive_config):
        """Should raise RuntimeError when repo_root context is not set."""
        with repo_root_context(temp_hive_config):
            # Create parent task first
            parent_id = create_task(title="Parent Task", hive_name="backend")

        # Now try to create subtask outside context
        with pytest.raises(RuntimeError, match="repo_root not set in context"):
            create_subtask(
                title="Test Subtask",
                parent=parent_id,
