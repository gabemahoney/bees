...
class TestGetTicketPath:
    ...
    def test_accepts_any_ticket_type(self, multi_hive, monkeypatch):
        ...
        assert path == backend_dir / "backend.bees-250.md"

    def test_flat_storage_path_for_epic(self, multi_hive, monkeypatch):
        """Should return flat storage path in hive root for epic."""
        repo_root, backend_dir, frontend_dir = multi_hive
        monkeypatch.chdir(repo_root)
        path = get_ticket_path("backend.bees-abc", "epic")
        assert path == backend_dir / "backend.bees-abc.md"

    def test_flat_storage_path_for_task(self, multi_hive, monkeypatch):
        """Should return flat storage path in hive root for task."""
        repo_root, backend_dir, frontend_dir = multi_hive
        monkeypatch.chdir(repo_root)
        path = get_ticket_path("backend.bees-xyz", "task")
        assert path == backend_dir / "backend.bees-xyz.md"

    def test_flat_storage_path_for_subtask(self, multi_hive, monkeypatch):
        """Should return flat storage path in hive root for subtask."""
        repo_root, backend_dir, frontend_dir = multi_hive
        monkeypatch.chdir(repo_root)
        path = get_ticket_path("frontend.bees-123", "subtask")
        assert path == frontend_dir / "frontend.bees-123.md"

    def test_flat_storage_ignores_ticket_type(self, multi_hive, monkeypatch):
        ...
        assert epic_path == backend_dir / "backend.bees-abc.md"
...
class TestInferTicketTypeFromId:
    ...
    def test_infers_epic_from_yaml_frontmatter(self, multi_hive, monkeypatch):
    ...
    def test_infers_task_from_yaml_frontmatter(self, multi_hive, monkeypatch):
    ...
    def test_infers_subtask_from_yaml_frontmatter(self, multi_hive, monkeypatch):
